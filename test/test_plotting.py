...
def test_plot_line():
    ...
    p.add_line((sympy.Integer(0), sympy.Integer(0)), (sympy.Integer(1), sympy.Integer(1)), "blue")
...
def test_plot_arrow():
    p = symfem.plotting.Picture()
    p.add_arrow(
        (sympy.Integer(0), sympy.Integer(0)), (sympy.Integer(1), sympy.Integer(1)), "orange"
    )
    for ext in ["svg", "png", "tex"]:
...
def test_plot_fill():
    ...
    p.add_fill(
        (
            ...
            (sympy.Rational(1, 2), sympy.Integer(0)),
        ),
        "purple",
        0.5,
    )
...
def test_dof_diagrams_lagrange(reference, degree):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_dof_diagram(
            os.path.join(folder, f"test_dof_diagrams_lagrange-{reference}-{degree}.{ext}")
        )
...
def test_dof_diagrams_raviart_thomas(reference, degree):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_dof_diagram(
            os.path.join(folder, f"test_dof_diagrams_raviart_thomas-{reference}-{degree}.{ext}")
        )
...
def test_dof_diagrams_eg(reference, degree):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_dof_diagram(os.path.join(folder, f"test_dof_diagrams_eg-{reference}-{degree}.{ext}"))
...
def test_function_plots_lagrange(reference, degree):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_basis_function(
            0, os.path.join(folder, f"test_function_plots_lagrange-{reference}-{degree}.{ext}")
        )
...
def test_function_plots_raviart_thomas(reference, degree):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_basis_function(
            0,
            os.path.join(folder, f"test_function_plots_raviart_thomas-{reference}-{degree}.{ext}"),
        )
...
def test_function_plots_piecewise_scalar(reference):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_basis_function(
            0, os.path.join(folder, f"test_function_plots_piecewise_scalar-{reference}.{ext}")
        )
...
def test_function_plots_dual(n):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_basis_function(0, os.path.join(folder, f"test_function_plots_dual-{n}.{ext}"))
...
def test_function_plots_eg(reference, degree):
    ...
    for ext in ["svg", "png", "tex"]:
        e.plot_basis_function(
            0, os.path.join(folder, f"test_function_plots_eg-{reference}-{degree}.{ext}")
        )
...
def test_plot_reference(reference):
    ...
    for ext in ["svg", "png", "tex"]:
        r.plot_entity_diagrams(os.path.join(folder, f"test_plot_reference-{rname}.{ext}"))
    compile_tex(f"test_plot_reference-{rname}.tex")


def test_metadata():
    img = symfem.plotting.Picture(
        svg_metadata=(
            "<metadata id='license'>\n"
