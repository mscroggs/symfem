...
def make_into_value_type(poly, element):
    ...
    if element.value_type == "vector":
        return [
            VectorFunction([p if i == j else 0 for j in range(element.range_dim)])
            for i in range(element.range_dim)
            for p in poly
        ]
    if element.value_type == "matrix":
        return [
            MatrixFunction(
                [
                    [p if i0 == j0 and i1 == j1 else 0 for j1 in range(element.range_shape[1])]
                    for j0 in range(element.range_shape[0])
                ]
            )
            for i0 in range(element.range_shape[0])
            for i1 in range(element.range_shape[1])
            for p in poly
        ]
    if element.value_type == "symmetric matrix":
        ...
        return [
            MatrixFunction(
                [
                    [
                        p if (i0 == j0 and i1 == j1) or (i1 == j0 and i0 == j1) else 0
                        for j1 in range(element.range_shape[1])
                    ]
                    for j0 in range(element.range_shape[0])
                ]
            )
            for i0 in range(element.range_shape[0])
            for i1 in range(i0, element.range_shape[1])
            for p in poly
        ]
...
def run_superdegree_test(element, superdegree, pfunc, pargs):
    ...
    poly = pfunc(
        *[
            (10 if superdegree is None else superdegree - 1) if p == "SUPERDEGREE" else p
            for p in pargs
        ]
    )
    poly_coeff = [
        polydict(p, element, 10 if superdegree is None else None)
        for p in make_into_value_type(poly, element)
    ]
...
def test_degrees(elements_to_test, cells_to_test, cell_type, element_type, order, kwargs, speed):
    ...
    run_subdegree_test(
        element, element.lagrange_subdegree, polynomial_set, (cell_type, "SUBDEGREE")
    )
    run_subdegree_test(
        element,
        element.polynomial_subdegree,
        polynomial_set_1d,
        (element.reference.tdim, "SUBDEGREE"),
    )
    run_superdegree_test(
        element, element.lagrange_superdegree, polynomial_set, (cell_type, "SUPERDEGREE")
    )
    run_superdegree_test(
        element,
        element.polynomial_superdegree,
        polynomial_set_1d,
        (element.reference.tdim, "SUPERDEGREE"),
    )
