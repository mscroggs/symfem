...
def to_2d(x, y, z):
    return (80 + (x + (-z+y/2-3)/2) * size, 58 + (3-y - z) * root3over2 * size)
...
for x in range(-3,26):
    for y in range(-3,6):
        ...
        for p2 in [(x+1,y), (x,y+1)]:
            z1 = zzz if p1 in all_ups else 0
            z2 = zzz if p2 in all_ups else 0
            z3 = zzz if p3 in all_ups else 0
